...
export function ToolCard({ tool }: { tool: Tool }) {
  return (
    <Link href={`/tools/${tool.slug}`} className="block h-full group outline-none">
      <Card className="relative h-full bg-background/50 backdrop-blur-xl border border-foreground/20 ring-0 overflow-hidden transition-all duration-500 hover:-translate-y-1 hover:shadow-2xl hover:shadow-primary/20 rounded-[1.5rem] group focus-visible:ring-2 focus-visible:ring-primary">
