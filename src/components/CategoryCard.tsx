...
export function CategoryCard({ category, count }: { category: Category; count: number }) {
  return (
    <Link href={`/category/${category.slug}`} className="block h-full group">
      <Card className="h-full bg-background/50 backdrop-blur-md border border-foreground/20 ring-0 transition duration-300 hover:border-primary/50 hover:bg-card/80 hover:-translate-y-1 hover:shadow-2xl hover:shadow-primary/20 rounded-[1.5rem] relative overflow-hidden group">
