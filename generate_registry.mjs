...
function toComponent(text) {
  const parts = text.replace(/[^a-zA-Z0-9 ]/g, '').split(' ');
  return parts.map(p => p.charAt(0).toUpperCase() + p.slice(1)).join('') + (text.toLowerCase().includes('calculator') ? '' : 'Calculator');
}
...
for (const line of lines) {
  ...
  if (tLine.startsWith('### ')) {
    currentCategory = tLine.substring(4).toLowerCase();
    currentCategorySlug = currentCategory.split(' ')[0]; // rough matching
    if (currentCategory.includes('fraction')) currentCategorySlug = 'fraction';
    else if (currentCategory.includes('percent')) currentCategorySlug = 'percent';
    else if (currentCategory.includes('probability')) currentCategorySlug = 'probability-statistics';
    else if (currentCategory.includes('time')) currentCategorySlug = 'time-date';
    else if (currentCategory.includes('construction')) currentCategorySlug = 'construction';
    else if (currentCategory.includes('word')) currentCategorySlug = 'word-problems';
    else if (currentCategory.includes('fun')) currentCategorySlug = 'fun-games';
    else if (currentCategory.includes('misc')) currentCategorySlug = 'miscellaneous';
  } else if (tLine.startsWith('- ')) {
    const name = tLine.substring(2);
    let slug = toSlug(name);
    // Overrides for top 20 exact matches if needed
    if (name === "GCD / Greatest Common Factor Calculator") slug = "gcd-lcm";
    if (slug === "temperature-conversion-calculator-c-f-k-r") slug = "temperature-converter";
    
    tools.push({
      slug,
      name: name,
      description: "Free online " + name.toLowerCase() + ".",
      category: currentCategorySlug,
      tags: [currentCategorySlug, ...name.toLowerCase().split(' ').filter(w => w.length > 3).map(w => w.replace(/[^a-z]/g, ''))].filter(Boolean),
...
for (const tool of tools) {
  let origSlug = tool.slug;
  let counter = 1;
  while (slugs.has(tool.slug)) {
    tool.slug = origSlug + '-' + counter;
    counter++;
  }
  slugs.add(tool.slug);
}

const fileContent = "import { Category, Tool } from '@/types/tool';\n\n" +
  "export const categories: Category[] = " + JSON.stringify(categories, null, 2) + ";\n\n" +
  "export const tools: Tool[] = " + JSON.stringify(tools, null, 2) + ";\n";



fs.writeFileSync('./src/lib/tools-registry.ts', fileContent, 'utf8');
